    ...
    <div className="scan-page flex flex-col items-center pt-4 sm:pt-8 px-2 sm:px-4 relative pb-20">
      ...
      <div className="mt-6 flex flex-col items-center gap-3">
        ...
        {!allDone && totalScanned > 0 && (
          <button
            onClick={async () => {
              const dest = order.isAddition ? "Additions" : "In Stock";
              if (!window.confirm(`Move order ${order.orderNumber} to ${dest}?\n\nOnly ${totalScanned} of ${totalQty} items scanned.`)) return;
              try {
          ...
          >
            Move to {order.isAddition ? "Additions" : "In Stock"} ({totalScanned}/{totalQty} scanned)
          </button>
        )}
      </div>
    </div>
