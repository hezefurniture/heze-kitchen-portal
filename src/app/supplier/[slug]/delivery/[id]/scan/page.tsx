...
export default function ScanPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/scan?supplierId=${supplierId}&deliveryId=${deliveryId}`);
  ...
  // Reload when scans are flushed from queue
  useEffect(() => {
  ...
  return (
    <div className="scan-page flex flex-col items-center pt-4 sm:pt-8 px-2 sm:px-4 relative pb-20">
      ...
      <p className="text-white/80 mb-8 text-lg">Scan the items to accept kitchens to the warehouse</p>
            ...
            <div key={order.id} className="fade-in">
              <button onClick={() => setExpandedOrder(isExpanded ? null : order.id)} className="w-full bg-card rounded-lg px-6 py-4 flex items-center justify-between hover:bg-card-light transition">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-white text-lg">DELIVERED {orderScanned}/{orderTotal}</span>
      ...
      {orders.length > 0 && (
        <button onClick={() => router.push(`/supplier/${slug}/delivery/${deliveryId}/finished`)} className="mt-8 px-16 py-4 rounded-lg border-2 border-accent text-white font-bold text-2xl hover:bg-accent/20 transition">Finish</button>
      )}
    </div>
  );
}
